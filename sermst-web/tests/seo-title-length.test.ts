...
import { metadata as pcmsoMetadata } from '@/app/saude/pcmso-programa-controle-medico/page';
...
describe('SEO title length', () => {
  ...
  it('mantem curto o title do guia de PCMSO', () => {
    const title = getTitleText(pcmsoMetadata.title);

    expect(title).toBe('PCMSO: o que é, quem precisa e qual a validade do programa');
