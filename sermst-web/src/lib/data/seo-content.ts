...
export const servicosSEO: Record<string, SEODocument> = {
  ...
  "audiometria-ocupacional-clinica": {
    ...
    hook:
      "Proteja a audição da equipe e reduza exposição a passivos relacionados a ruído ocupacional. A SERMST realiza audiometria ocupacional integrada ao PCMSO com estrutura adequada e foco em validade técnica.",
    isClinico: true,
    quandoRequerido:
      "A audiometria ocupacional é indicada para funções expostas a ruído acima dos limites de tolerância e costuma ser exigida em admissão, demissão e acompanhamentos periódicos.",
    content: {
      dor:
      ...
      solucao:
        "A SERMST realiza audiometria com critério técnico, profissionais especializados e integração ao fluxo do ASO para dar mais consistência ao monitoramento ocupacional.",
      beneficios: [
...
export const dicionarioSEO: Record<string, SEODocument> = {
  ...
  "o-que-e-pgr": dicionarioEntry(
    ...
    "PGR é o Programa de Gerenciamento de Riscos exigido pela NR-01. Ele organiza a identificação, avaliação e controle dos riscos ocupacionais da empresa.",
    "Muita empresa ainda confunde PGR com documento padrão ou com o antigo PPRA, o que enfraquece a coerência técnica da rotina de SST.",
    "Entender o PGR ajuda a conectar risco real da operação com laudos, exames, treinamentos e eventos do eSocial.",
  ),
  "o-que-e-ltcat": dicionarioEntry(
    "O que é LTCAT: significado do laudo técnico das condições ambientais do trabalho",
    "LTCAT é o Laudo Técnico das Condições Ambientais do Trabalho, usado para documentar agentes nocivos e sustentar PPP e aposentadoria especial.",
    "Sem entender o papel do LTCAT, a empresa mistura conceitos trabalhistas e previdenciários e gera documento inconsistente para perícia, PPP e eSocial.",
    "A definição correta ajuda a separar o que é insalubridade trabalhista, o que é exposição previdenciária e onde o LTCAT entra de fato.",
  ),
