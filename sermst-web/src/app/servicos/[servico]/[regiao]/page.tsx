...
function getLocalServiceDescription(servico: string, serviceName: string, local: (typeof localidades)[number]) {
  ...
  if (servico === 'exame-toxicologico-clt') {
    if (local.slug === 'sao-paulo') {
      return 'Exame toxicológico em São Paulo Centro, no Largo do Paissandu, com atendimento para empresas e pessoa física. A SERMST apoia CNH C, D e E, admissão, demissão e quem busca exame toxicológico perto de mim em São Paulo.';
    }
  ...
  if (servico === 'audiometria-ocupacional-clinica' || servico === 'exames-complementares-laboratoriais') {
    if (servico === 'audiometria-ocupacional-clinica' && local.slug === 'sao-paulo') {
      return 'Audiometria ocupacional em São Paulo Centro, no Largo do Paissandu, para empresas que buscam clínica de audiometria perto de mim, resultado integrado ao ASO e apoio ao PCMSO.';
    }
          ...
          <div className="mx-auto w-full max-w-[1280px] px-6">
            <div className="mx-auto max-w-5xl rounded-[2rem] border border-slate-200 bg-slate-50 p-8 lg:p-10">
              ...
              <div className="grid gap-6 md:grid-cols-3">
                <Link
                  href={`/servicos/exame-admissional-expresso/${local.slug}`}
                ...
                <Link
                  href={`/servicos/exames-complementares-laboratoriais/${local.slug}`}
                  className="group rounded-[1.5rem] border border-slate-200 bg-white p-6 transition-all hover:-translate-y-1 hover:border-accent-pink/30 hover:shadow-lg"
                >
                  <h3 className="mb-3 text-xl font-black text-brand-900 group-hover:text-accent-pink">
