  ...
  return (
    <div className="not-prose space-y-6">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-900 text-white">
              ...
              <th className="px-4 py-3 text-center text-xs font-black uppercase tracking-wider whitespace-nowrap">RAT/GILRAT*</th>
            </tr>
          </thead>
          <tbody>
            {paginated.length === 0 ? (
              ...
              paginated.map((entry, i) => (
                <tr key={entry.codigo} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                  ...
                  <td className="px-4 py-3 text-center text-xs font-bold text-slate-700 whitespace-nowrap">
                    {entry.ratGilrat ? `${entry.ratGilrat}%` : '—'}
                  </td>
                </tr>
              ))
            )}
      ...
      <p className="text-xs leading-relaxed text-slate-500">
        * Alíquota do Anexo V do Decreto 3.048 para cada CNAE. A apuração deve considerar o CNAE da atividade econômica preponderante e o FAP da empresa.
      </p>
